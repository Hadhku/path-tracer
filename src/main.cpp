...
int main( int argc, char* argv[] )
{
	...
	std::vector<std::unique_ptr<Integrator::PT>> integrator;
	// Use NEE, false for basic basic PT
	bool const f_NEE{ false };
	for ( std::uint8_t i{ 0 }; i < omp_get_max_threads(); ++i )
		integrator.emplace_back( std::make_unique<Integrator::PT>( camera, sensor, scene, config, f_NEE ) );
